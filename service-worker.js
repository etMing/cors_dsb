...
function clearAllRules() {
  console.log('[CORS DSB] clearAllRules() called');
  return chrome.declarativeNetRequest.getDynamicRules().then(function(existing) {
    console.log('[CORS DSB] clearAllRules: ' + existing.length + ' existing DNR rules');
    if (existing.length === 0) {
      activeRuleIds = [];
      return;
    }
    var ids = existing.map(function(r) { return r.id; });
    console.log('[CORS DSB] clearAllRules: removing ' + ids.length + ' rules: [' + ids.join(',') + ']');
    return chrome.declarativeNetRequest.updateDynamicRules({ removeRuleIds: ids }).then(function() {
      console.log('[CORS DSB] clearAllRules: done, all DNR rules removed');
      activeRuleIds = [];
    });
  }).catch(function(err) {
    console.error('[CORS DSB] clearAllRules error:', err);
  });
}

function registerRules(isStartup) {
  console.log('[CORS DSB] registerRules() called: enabled=' + enabled + ', rules.length=' + rules.length +
    ', isStartup=' + !!isStartup);
  ...
  console.log('[CORS DSB] registerRules: effectiveRules=' + effectiveRules.length + ' (enabled=' + enabled +
    ', total=' + rules.length + ')');
  ...
  if (effectiveRules.length === 0) {
    ...
    if (isStartup) {
      return chrome.declarativeNetRequest.getDynamicRules().then(function(existing) {
        if (existing.length > 0) {
          console.warn('[CORS DSB] SAFETY: effectiveRules=0 on startup but ' + existing.length +
            ' DNR rules exist — PRESERVING (storage may not be ready).');
          activeRuleIds = existing.map(function(r) { return r.id; });
          return;
        }
        console.log('[CORS DSB] registerRules: no effective rules, no existing rules — nothing to do');
        activeRuleIds = [];
      });
    }
    // Normal operation: user intentionally cleared/disabled all rules
    console.log('[CORS DSB] registerRules: no effective rules — clearing all DNR rules');
    return clearAllRules();
  }

  var result = generateAllDnrRules(effectiveRules);
  var dnrRules = result.dnrRules;

  if (dnrRules.length === 0) {
    console.log('[CORS DSB] registerRules: generateAllDnrRules produced 0 DNR rules from ' +
      effectiveRules.length + ' user-rules');
    if (isStartup) {
      return chrome.declarativeNetRequest.getDynamicRules().then(function(existing) {
        if (existing.length > 0) {
          console.warn('[CORS DSB] SAFETY: dnrRules=0 on startup but ' + existing.length +
  ...
  console.log('[CORS DSB] registerRules: generated ' + dnrRules.length + ' DNR rules, checking against existing...');
  ...
  return chrome.declarativeNetRequest.getDynamicRules().then(function(existingRules) {
    console.log('[CORS DSB] registerRules: fetched ' + existingRules.length + ' existing DNR rules');
    ...
    if (diff.toAdd.length === 0 && diff.toRemove.length === 0) {
      ...
      console.log('[CORS DSB] registerRules: DNR rules unchanged (' + dnrRules.length +
        ' active), skipping update');
      activeRuleIds = dnrRules.map(function(r) { return r.id; });
      return;
    }

    console.log('[CORS DSB] registerRules: diff +' + diff.toAdd.length + ' -' + diff.toRemove.length);
    if (diff.toRemove.length > 0) {
      console.log('[CORS DSB] registerRules: removing IDs [' + diff.toRemove.join(',') + ']');
    }
    ...
    return chrome.declarativeNetRequest.updateDynamicRules({
      ...
      activeRuleIds = dnrRules.map(function(r) { return r.id; });
      console.log('[CORS DSB] registerRules: DNR update SUCCESS, ' + activeRuleIds.length + ' rules active');
    }).catch(function(err) {
      console.error('[CORS DSB] registerRules: updateDynamicRules FAILED:', err);
      throw err;
    });
  }).catch(function(err) {
    console.error('[CORS DSB] registerRules error:', err);
...
function updateIcon() {
  ...
  try {
    ...
    chrome.action.setTitle({ title: 'CORS DSB' });
  } catch (e) { /* ignore */ }
}
    ...
    return chrome.storage.local.get(['rules']).then(function(localData) {
      ...
      try {
        if (typeof localData.rules === 'string' && localData.rules) {
          ...
          console.log('[CORS DSB] loadSettings: parsed ' + rules.length + ' rules from JSON string');
        } else if (Array.isArray(localData.rules)) {
          rules = localData.rules;
          console.log('[CORS DSB] loadSettings: loaded ' + rules.length + ' rules from array');
        } else {
          rules = [];
          console.log('[CORS DSB] loadSettings: no rules found in local storage (rules=[])');
        }
      } catch (e) {
        console.error('[CORS DSB] Failed to parse rules:', e);
        rules = [];
      }
      ...
      if (rules.length === 0) {
        try {
          if (typeof pref.rules === 'string' && pref.rules) {
            ...
            if (rules.length > 0) {
              console.log('[CORS DSB] Migrated ' + rules.length + ' rules from sync to local storage');
            }
          ...
          console.error('[CORS DSB] Failed to parse migrated rules:', e2);
        }
      }
      ...
      console.log('[CORS DSB] loadSettings() complete: ' + rules.length + ' rules, changed=' + changed);
      return { pref: pref, changed: changed };
    });
  }).catch(function(err) {
    console.error('[CORS DSB] loadSettings() FAILED:', err);
    throw err;
  });
}
...
function addRule(ruleData) {
  var rule = {
    ...
    cors: normalizeCors(ruleData.cors),
...
function updateRule(ruleId, ruleData) {
  ...
  var updated = {
    ...
    cors: ruleData.cors !== undefined ? normalizeCors(ruleData.cors) : rules[idx].cors,
...
function init() {
  console.log('[CORS DSB] init() called, g_initPromise=' + (g_initPromise ? 'pending' : 'null'));
  ...
  g_initPromise = loadSettings().then(function(result) {
    console.log('[CORS DSB] init: loadSettings resolved, result.changed=' + result.changed);
    if (result.changed) {
      console.log('[CORS DSB] init: saving rules (IDs were assigned)');
      return saveRules();
    }
  }).then(function() {
    console.log('[CORS DSB] init: loading enabled state from storage...');
    return chrome.storage.local.get(['enabled']);
  }).then(function(local) {
    enabled = local.enabled === true;
    console.log('[CORS DSB] init: enabled=' + enabled + ', calling registerRules...');
    updateIcon();
    return registerRules(true);
  }).then(function() {
    console.log('[CORS DSB] init: COMPLETE. Global:', enabled,
      '| Rules:', rules.length,
    ...
    console.error('[CORS DSB] init: FAILED:', err);
...
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse) {
  ...
  if (message.type === 'replaceAllRules') {
    ...
    rules = message.rules || [];
    // Ensure all have IDs and normalized cors
    rules.forEach(function(rule) {
      if (!rule.id) rule.id = generateRuleId();
      rule.cors = normalizeCors(rule.cors);
    });
...
chrome.runtime.onInstalled.addListener(function(details) {
  console.log('[CORS DSB] onInstalled fired, reason=' + (details && details.reason));
  init();
});

chrome.runtime.onStartup.addListener(function() {
  console.log('[CORS DSB] onStartup fired — browser launched, initializing');
  init();
});
...
console.log('[CORS DSB] Service worker top-level execution, calling init()');
init();
